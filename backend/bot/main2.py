...
async def url(message: types.Message):
    ...
    try:
        access_ids.remove(message.from_user.id)
    except KeyError:
        pass
...
async def set_liter(callback: types.CallbackQuery):
    ...
    await bot.send_message(
        ...
        '\n\n🔑 Для каждого ученика предназначен свой уникальный код для входа в игру. В полученной таблице вы можете '
        'сами добавить столбец с фамилиями учеников и затем передать его ученикам (отправить таблицу в чат класса, '
        'распечатать и передать ученикам и другое).',
    )
    ...
    await bot.send_message(
        ...
        '\n\nДля получения дополнительных кодов КЛИКНИТЕ по команде /start в этом сообщении.'
        '\n\nСкачайте файл с кодами для каждого ученика на свой компьютер или телефон.',
        )
