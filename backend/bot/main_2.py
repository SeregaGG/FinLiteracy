...
async def contact(message):
    ...
    await bot.send_message(message.from_user.id, "Выберите ваш город", reply_markup=inline_kb)



# @dp.callback_query_handler(text=['Екатеринбург', 'Челябинск', 'Березовский'])
async def set_city(callback: types.CallbackQuery):
    if not callback.from_user.id in access_ids:
        await bot.send_message(callback.message.chat.id, "Введите пароль, который вам предоставили")
        return 0
    if callback.message.contact is not None:
        print(callback.message.contact)
    if not callback.data in cities:
    ...
    user_data.update(current_user_data)
    await bot.send_message(callback.message.chat.id, "Выберите вашу школу", reply_markup=inline_school)
...
async def set_school(callback: types.CallbackQuery):
    ...
    user_data.update(current_user_data)
    await bot.send_message(callback.message.chat.id, "Выберите номер вашего класса", reply_markup=class_number_inline)
    await callback.answer()
...
async def set_number(callback: types.CallbackQuery):
    ...
    user_data.update(current_user_data)
    await bot.send_message(callback.message.chat.id, "Выберите букву вашего класса", reply_markup=inline_class_liter)
    await callback.answer()
...
async def set_liter(callback: types.CallbackQuery):
    ...
    await bot.send_message(callback.from_user.id, f'Код доступа для учеников - {random_str}')
...
dp.register_callback_query_handler(set_city, state='*', text=['Екатеринбург', 'Челябинск', 'Березовский'])
dp.register_callback_query_handler(set_school, state='*', text=['МОУ СОШ №110', 'МОУ СОШ №111', 'МОУ СОШ №112'])
dp.register_callback_query_handler(set_number, state='*', text=['1', '2', '3', '4', '5'])
dp.register_callback_query_handler(set_liter, state='*', text=['А', 'Б', 'В', 'Г', 'Д', 'Е', 'Ж'])
